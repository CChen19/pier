...
- **D103**: *能效状态持久化与独立闭环遥测* —— 效率决策状态随会话分支恢复（D38 范式）；每个能效机制提供独立的 `logEnabled` 开关与独立 JSONL 审计日志，记录每轮决策参数、预测盈亏平衡 vs 实测偿还轮次，提供运行时审计命令（D104 起统一为 `/pier-config show efficiency` / `/pier-config check`，原 `/efficiency` 已删除）。
